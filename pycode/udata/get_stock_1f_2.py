...
for n in range(4, 6):
    ...
    month = 2
    num = n*2+1
    last_day = calendar.monthrange(year, num+1)[1]
    start = datetime.date(year, num, 1).strftime('%Y%m%d')
    end = datetime.date(year, num+1, last_day).strftime('%Y%m%d')
    print(start)
    print(end)
    # for
    for i in range(len(data.hs_code)):
        ...
        print(dat)
        time.sleep(0.1)
        filename = "../data1/" + data.hs_code.iloc[i] + "_" + \
