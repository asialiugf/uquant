...
class ini_value {
   ...
   private:
    ...
    struct as_helper< ::util::config::duration_value, _TVOID> {
        inline static ::util::config::duration_value convert(const ini_value &val, size_t index) {
    ...
    struct as_helper<int8_t, _TVOID> {
        inline static int8_t convert(const ini_value &val, size_t index) {
            return val.as_int8(index);
        }
    };

    template <typename _TVOID>
    struct as_helper<uint8_t, _TVOID> {
        inline static uint8_t convert(const ini_value &val, size_t index) {
            return val.as_uint8(index);
        }
    };

    template <typename _TVOID>
    struct as_helper<int16_t, _TVOID> {
        inline static int16_t convert(const ini_value &val, size_t index) {
            return val.as_int16(index);
        }
    };

    template <typename _TVOID>
    struct as_helper<uint16_t, _TVOID> {
        inline static uint16_t convert(const ini_value &val, size_t index) {
            return val.as_uint16(index);
        }
    };

    template <typename _TVOID>
    struct as_helper<int32_t, _TVOID> {
        inline static int32_t convert(const ini_value &val, size_t index) {
            return val.as_int32(index);
        }
    };

    template <typename _TVOID>
    struct as_helper<uint32_t, _TVOID> {
        inline static uint32_t convert(const ini_value &val, size_t index) {
            return val.as_uint32(index);
        }
    };

    template <typename _TVOID>
    struct as_helper<int64_t, _TVOID> {
        inline static int64_t convert(const ini_value &val, size_t index) {
            return val.as_int64(index);
        }
    };

    template <typename _TVOID>
    struct as_helper<uint64_t, _TVOID> {
        inline static uint64_t convert(const ini_value &val, size_t index) {
            return val.as_uint64(index);
        }
    };

    template <typename _TVOID>
    struct as_helper<const char *, _TVOID> {
        inline static const char *convert(const ini_value &val, size_t index) {
            return val.as_string(index);
        }
    };

    template <typename _TVOID>
    struct as_helper<std::string, _TVOID> {
        inline static std::string convert(const ini_value &val, size_t index) {
            return val.as_cpp_string(index);
        }
    };
    ...
    struct as_helper {
        ...
        inline static _Tt convert(const ini_value &val, size_t index) {
...
class ini_loader {
   ...
   public:
     ...
     * @param index 转储索引，默认是第一个值
     */
    void dump_to(const std::string &path, std::string &val, bool is_force = false,
                 size_t index = 0);
     ...
     * @param index 转储索引，默认是第一个值
     */
    void dump_to(const std::string &path, char *begin, char *end, bool is_force = false,
                 size_t index = 0);
    ...
    void dump_to(const std::string &path, char (&val)[MAX_COUNT], bool is_force = false,
                 size_t index = 0) {
