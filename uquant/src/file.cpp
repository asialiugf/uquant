...
int SaveLine(const char *FileName, const char *pcLine) {
    ...
    while ((fd = open(FileName, O_WRONLY | O_CREAT | O_APPEND, 0660)) == -1) {
        sleep(1);
        if ((retry++) > 10) {
            return_code = strlen(FileName);
            if (return_code <= 0) {
                ErrLog(1000, "File Name is null!", 1, 0, 0);
                printf("File.cpp: file name is null !!! error_no is %d\n", errno);
                return -1;
            }
    ...
    return_code = write(fd, pcLine, strlen(pcLine));
    return_code = write(fd, "\n", 1);
    // return_code = write(fd,"\n",1);
    // return_code = write(fd,"\n",1);
    close(fd);
    return return_code;
...
Save_tick(char *buf, int len)
{
  ...
  sprintf(ca_tick_file,
"../../dat/rcv_dat/tick-%s-%s-%s.bin",market_data->InstrumentID, \
...
int disp_tick(char *buf)
{
  ...
  sprintf(ca_errtmp,"recv: %s----%s %s----%s----Msec:%s ---- %d ----
",market_data->InstrumentID, \
  ...
  sprintf(ca_errmsg,"%s  H:%s  L:%s  LPrice:%s  B1:%s  BV1:%s  A1:%s  AV1:%s
V:%s", \
          ca_errtmp, \
