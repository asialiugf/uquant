...
class CSimpleHandler : public CTraderSpi {
   public:
    ...
    virtual void OnFrontConnected() {
        ...
        strcpy(g_chInvestorID, reader.Get("config", "InvestorID", "").c_str());
        strcpy(g_chAuthCode, reader.Get("config", "AuthCode", "").c_str());
        strcpy(g_chAppID, reader.Get("config", "AppID", "").c_str());
        strcpy(g_chInstrumentID, reader.Get("config", "InstrumentID", "").c_str());
        strcpy(g_chExchangeID, reader.Get("config", "ExchangeID", "").c_str());
        ...
        // strcpy(g_chInstrumentID, getConfig("config", "InstrumentID").c_str());
        // strcpy(g_chExchangeID, getConfig("config", "ExchangeID").c_str());
        ...
        std::cout << "CTraderSpi  thread : ---------  " << this_id << " sleeping...ReqUserLogin\n";
    }
    ...
    void ReqQryDepthMarketData() {
        ...
        printf("请输入交易所 SHFE 的合约 :\n");
        printf("请输入合约号 ag2306 or fu2305?  查询所有合约请输入 0 :\n");
        cin >> New_instrument;
        if (New_instrument.c_str()[0] != '0') {
            strcpy(a.InstrumentID, New_instrument.c_str());
            strcpy(a.ExchangeID, "SHFE");
        }

        // strcpy(a.BankID, g_chBrokerID);
        int b = m_pUserApi->ReqQryDepthMarketData(&a, nRequestID++);
        printf((b == 0) ? "请求查询行情......发送成功\n" : "请求查询行情......发送失败，序号=[%d]\n", b);
    }
    ...
    void ReqAuthenticate() {
        ...
        strcpy(a.BrokerID, reader.Get("config", "BrokerID", "").c_str());
        strcpy(a.UserID, reader.Get("config", "UserID", "").c_str());
        strcpy(a.AuthCode, reader.Get("config", "AuthCode", "").c_str());
        strcpy(a.AppID, reader.Get("config", "AppID", "").c_str());

        // strcpy(a.UserProductInfo, g_chAppID);
        int b = m_pUserApi->ReqAuthenticate(&a, nRequestID++);
        printf("\t客户端认证 = [%d]\n", b);
        std::cout << "kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkffffffffffffffffffffffffff\n";
    }
    ...
    virtual void OnRspUserLogin(CThostFtdcRspUserLoginField *pRspUserLogin, CThostFtdcRspInfoField *pRspInfo, int nRequestID,
        ...
        // if (pRspInfo)
        {
            uBEE::gb18030_to_utf8(pRspInfo->ErrorMsg, ErrMsg3, 1024);
            printf(
                "\tFailed to login, errorcode=[%d]\n \terrormsg=[%s]\n "
                "\trequestid = [%d]\n \tchain = [%d]\n",
                pRspInfo->ErrorID, ErrMsg3, nRequestID, bIsLast);
            // exit(-1);
        }
        // SetEvent(g_hEvent);
    }
    ...
    void ReqOrderInsert_Ordinary() {
        ...
        // my_clear();
        string new_limitprice;
        std::cout << "交易合约instrumentID:" << g_chInstrumentID << " : " << std::endl;
        printf("请输入指定价格：\n");
        cin >> new_limitprice;
