...
int SEE_RSI(int start, int finish, const double in[], int N,
    ...
    */
    ...
    SEE_SMA(start, finish, &_max[0], N, 1, &_sma1[0]);
    SEE_SMA(start, finish, &_abs[0], N, 1, &_sma2[0]);
...
int SEE_EMA(int start, int finish, const double in[], /* outReal index and inReal index must be the same */
...
/* out 初始化为 SEE_NULL                       */
{
    ...
    if (N < 0 || N > ARRAY_SIZE) return -1;
    if (finish < start) return -1;
    if (SEE_LMA(start, finish, &in[0], 'E', N, 2, &out[0]) != 0) {
        return -1;
    }
    return 0;
}

int SEE_SMA(int start, int finish, const double in[], /* outReal index and inReal index must be the same */
...
/* out 初始化为 SEE_NULL                       */
{
    ...
    if (M > N) return -1;
    if (finish < start) return -1;
    if (SEE_LMA(start, finish, &in[0], 'S', N, M, &out[0]) != 0) {
        return -1;
    }
    return 0;
}
 ...
 *  6、 如果要计算第888处的MA的话，一般情况start和finish需输入为888，并且&in[0],和&out[0]开始。
 *      但是，如果N比较小，比如只有5，那可以输入为： N=5,start=5,finish=5, &in[888-5],&out[888-5]即可。
 *      这样的话，只需要存储 in[X-N]到in[X]，共N+1个值即可。
 */

int SEE_MA(int start, int finish, const double in[], /* outReal index and inReal index must be the same */
           int N,                                    /* From 0 to ARRAY_SIZE                                */
           double out[])                             /* out index and in index must be the same     */
/* out 初始化为 SEE_NULL                       */
{
...
int SEE_LMA(int start, int finish, const double in[], /* outReal index and inReal index must be the same */
...
//--------------------RSV--begin----------------------------------
int SEE_RSV(int start, int finish, const double H[], /* outReal index and inReal index must be the same */
